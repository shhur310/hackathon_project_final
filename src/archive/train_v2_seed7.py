...
XGB_PARAMS = {
    ...
    "tree_method": "hist", "random_state": 7, "n_jobs": -1, "verbosity": 0,
}
...
CAT_PARAMS = {
    ...
    "random_seed": 7, "verbose": False, "od_type": "Iter", "od_wait": 200,
    "thread_count": -1,
}
...
def main(catboost_only: bool = False, fold: int = 5):
    ...
    if not catboost_only:
        np.savez(MODELS / "oof_v2_seed7.npz",
                 lgbm=oof_lgbm, xgb=oof_xgb, cat=oof_cat, y=y.values)
        np.savez(MODELS / "test_v2_seed7.npz",
                 lgbm=test_pred_lgbm, xgb=test_pred_xgb, cat=test_pred_cat)
    else:
        np.savez(MODELS / "oof_v2_seed7.npz", cat=oof_cat, y=y.values)
        np.savez(MODELS / "test_v2_seed7.npz", cat=test_pred_cat)
    ...
    pd.DataFrame({"ID": sub["ID"].values, "probability": test_blend}).to_csv(
        SUB / "submission_v2_seed7.csv", index=False)
    ...
    (MODELS / "v2_summary_seed7.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2))
