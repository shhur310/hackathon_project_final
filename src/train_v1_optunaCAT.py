...
def main(catboost_only: bool = False, fold: int = 5):
    ...
    if not catboost_only:
        ...
        np.savez(MODELS / "oof_v1_optunaCAT.npz", cat=oof_cat, y=y.values)
        np.savez(MODELS / "test_v1_optunaCAT.npz", cat=test_pred_cat)
    ...
    pd.DataFrame({"ID": sub["ID"].values, "probability": test_blend}).to_csv(
        SUB / "submission_v1_optunaCAT.csv", index=False)
    ...
    (MODELS / "v1_optunaCAT_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2))
