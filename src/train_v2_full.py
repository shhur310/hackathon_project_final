...
def main(catboost_only: bool = False, fold: int = 5):
    ...
    if not catboost_only:
        ...
        np.savez(MODELS / "oof_v2.npz", cat=oof_cat, y=y.values)
        np.savez(MODELS / "test_v2.npz", cat=test_pred_cat)
    ...
    pd.DataFrame({"ID": sub["ID"].values, "probability": test_blend}).to_csv(
        SUB / "submission_v2.csv", index=False)
    ...
    (MODELS / "v2_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2))
